...
import AdminFoundationLabel from "./AdminFoundationLabel";
import {
  LayoutDashboard,
  MessageSquare,
  Users,
...
const STATUS_CFG: Record<string, { label: string; dot: string; pill: string; text: string }> = {
  ...
  REVIEW:    { label: "Under Review", dot: "bg-sky-500", pill: "bg-sky-500/10 border border-sky-500/20", text: "text-sky-400" },
  ASSIGNED:  { label: "Assigned",     dot: "bg-blue-500",   pill: "bg-blue-500/10 border border-blue-500/20",   text: "text-blue-400" },
...
const PRIORITY_CFG: Record<string, { label: string; pill: string; text: string }> = {
  ...
  MEDIUM:    { label: "Medium",    pill: "bg-blue-500/10 border border-blue-500/20",     text: "text-blue-400" },
...
const STATUS_CFG_LIGHT: Record<string, { dot: string; pill: string; text: string }> = {
  ...
  REVIEW:    { dot: "bg-sky-500", pill: "bg-sky-50 border border-sky-200", text: "text-sky-700" },
  ASSIGNED:  { dot: "bg-blue-500",   pill: "bg-blue-50 border border-blue-200",   text: "text-blue-700" },
...
const PRIORITY_CFG_LIGHT: Record<string, { pill: string; text: string }> = {
  ...
  MEDIUM:    { pill: "bg-blue-50 border border-blue-200",       text: "text-blue-700" },
...
function StatCard({ title, value, sub, icon, accent, isDark, delay = 0 }: StatCardProps) {
  const card = isDark ? "medical-surface-card border" : "bg-white border-slate-200";
  return (
    <motion.div
...
function DeleteConfirmDialog({
  ...
  const cardBg = isDark ? "medical-surface-card border" : "bg-white border-slate-200";
            ...
            <div>
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-medium border transition-all ${
                        selected
                          ? `${cfg.pill} ${cfg.text} ring-2 ring-offset-1 ${
                              isDark ? "ring-blue-400/60 ring-offset-slate-900" : "ring-blue-500/50 ring-offset-white"
                            }`
                          : isDark
              ...
              <button
                ...
                className={`mt-3 px-4 py-2 rounded-xl text-xs font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                  isDark
                    ? "bg-gradient-to-r from-blue-500 to-sky-400 hover:from-blue-400 hover:to-sky-300 text-slate-950 shadow-md shadow-blue-500/20"
                    ...
                    <div>
                      ...
                      <a
                        ...
                        className={`inline-flex items-center gap-2 text-sm px-3 py-2 rounded-lg border transition-colors ${
                          isDark ? "border-slate-700 text-blue-400 hover:bg-slate-800" : "border-slate-200 text-blue-600 hover:bg-slate-50"
                        }`}
                      >
                        <Paperclip size={14} />
                        View Attachment
...
function LoginScreen({
  ...
  const bg = isDark ? "medical-text-primary" : "bg-[#f4f7f6]";
  const card = isDark
    ? "medical-surface-card border shadow-2xl shadow-black/40"
    : "bg-white border-slate-200/80 shadow-xl shadow-blue-900/5";
  const inp = isDark
    ? "medical-surface-input border-2 placeholder:text-slate-400 focus:border-blue-400 focus:ring-4 focus:ring-blue-500/20"
    : "bg-[#fbfdfd] border-slate-200 text-slate-800 placeholder-slate-400 focus:border-blue-600";
  ...
  return (
    <div className={`min-h-screen ${bg} flex items-center justify-center p-4 relative overflow-hidden`}>
      {/* Ambient medical blue glow — matches main page */}
      {isDark ? (
        <>
          <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-blue-500/5 rounded-full blur-[130px] pointer-events-none" />
          <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-sky-500/5 rounded-full blur-[120px] pointer-events-none" />
        </>
      ) : (
        <>
          <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-blue-400/10 rounded-full blur-[120px] pointer-events-none" />
          <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-sky-400/5 rounded-full blur-[110px] pointer-events-none" />
        </>
      )}
      {/* Dot grid */}
      <div className={`absolute inset-0 pointer-events-none opacity-[0.025] ${isDark ? "bg-[radial-gradient(#60a5fa_1px,transparent_1px)]" : "bg-[radial-gradient(#1d4ed8_1.5px,transparent_1.5px)]"} [background-size:24px_24px]`} />
        ...
        <div className="flex flex-col items-center gap-3 mb-8">
          <div className="text-center w-full">
            <div className={`flex items-center justify-center gap-1.5 mb-3`}>
              ...
              <span className={`text-[9px] font-extrabold uppercase tracking-widest ${isDark ? "text-blue-400/80" : "text-blue-600"}`}>
                Admin Panel
              </span>
            </div>
            <HospitalHeaderBranding variant="login" isDark={isDark} logoPriority />
          </div>
        </div>
        ...
        >
          ...
          <button
            ...
            className={`w-full py-2.5 rounded-xl text-sm font-bold transition-all disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-2 active:scale-[0.98] ${
              isDark
                ? "bg-gradient-to-r from-blue-500 to-sky-400 hover:from-blue-400 hover:to-sky-300 text-slate-950 shadow-lg shadow-blue-500/20"
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-2.5 rounded-lg font-medium transition-all duration-150 ${
        ...
        childActive
          ? isDark
            ? "bg-gradient-to-r from-blue-500 to-sky-400 text-slate-950 shadow-md shadow-blue-500/20"
...
function Sidebar({
  ...
  const sidebarBg = isDark ? "medical-surface-overlay border-r" : "bg-white border-slate-200";
  ...
  const hospitalTitleClass = `text-sm sm:text-base md:text-lg font-extrabold uppercase tracking-wide leading-none truncate ${
    isDark
  ...
  return (
    ...
    >
      {/* Hospital Logo */}
      <div
        className={`shrink-0 flex items-center px-3 py-4 border-b ${collapsed ? "justify-center" : "gap-3"} ${isDark ? "border-slate-700" : "border-slate-100"}`}
      >
        <AppLogo size="sidebar" className={collapsed ? "!h-9 !max-w-[36px] !border-2" : undefined} />
        {!collapsed && (
          <div className="min-w-0 flex flex-col justify-center gap-0 h-14 sm:h-16 md:h-[72px] leading-none">
            <div className={hospitalTitleClass}>RLG Eye</div>
            <div className={hospitalTitleClass}>Hospital</div>
            <div
              className={`text-[10px] sm:text-xs md:text-sm leading-none truncate ${
          ...
          <div className="flex flex-col">
            ...
            <button
              ...
              className={`w-full text-left px-2.5 py-2.5 rounded-xl border transition-all duration-150 ${
                foundationActive
                  ? isDark
                    ? "bg-slate-800/90 border-blue-500/40 text-blue-100"
            ...
            >
              <div className="flex items-start gap-2">
                <div className="relative shrink-0 mt-0.5">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold bg-gradient-to-tr from-blue-500 to-sky-400 text-slate-950`}>
                    {user.username?.[0]?.toUpperCase() ?? "A"}
                  </div>
                  <span
                    className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full bg-sky-400 border-2 ${
                ...
                {!collapsed && (
                  <>
                    <div className="flex-1 min-w-0 pr-1">
                      <AdminFoundationLabel isDark={isDark} variant="sidebar" />
                      <div className={`text-[10px] mt-0.5 ${isDark ? "text-blue-400/90" : "text-blue-600"}`}>
                        @RLG
                      </div>
...
export default function AdminDashboardPage() {
  ...
  // ─── Derived colors ────────────────────────────────────────────────────────
  const bg = isDark ? "medical-text-primary" : "bg-[#f4f7f6] text-slate-900";
  const cardBg = isDark
  ...
  if (authLoading) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${isDark ? "medical-text-primary" : "bg-[#f4f7f6]"}`}>
        ...
        >
          <div className={`px-5 py-4 border-b ${divider} flex items-center justify-between`}>
            ...
            <button
              ...
              className={`text-xs font-medium ${isDark ? "text-blue-400 hover:text-blue-300" : "text-blue-600 hover:text-blue-700"} flex items-center gap-1`}
            >
                ...
                {dataLoading ? (
                  ...
                  (complaints ?? []).slice(0, 5).map((c) => (
                    <tr key={c.id} className={`${tableHover} transition-colors`} onClick={() => setSelected(c)}>
                      <td className="px-4 py-3 font-mono text-xs text-blue-500">{c.ticket_id}</td>
              ...
              <tbody className={`divide-y ${divider}`}>
                {dataLoading ? (
                  ...
                  (complaints ?? []).map((c) => (
                    ...
                    >
                      <td className="px-4 py-3 font-mono text-xs text-blue-500 whitespace-nowrap">{c.ticket_id}</td>
                      {isIndividual && (
  ...
  return (
    <div className={`flex h-dvh min-h-dvh ${bg} font-sans relative overflow-hidden`}>
      ...
      {isDark ? (
        <>
          <div className="fixed top-0 left-1/4 w-[600px] h-[600px] bg-blue-500/[0.04] rounded-full blur-[140px] pointer-events-none z-0" />
          <div className="fixed top-1/3 right-1/4 w-[500px] h-[500px] bg-sky-500/[0.03] rounded-full blur-[130px] pointer-events-none z-0" />
        </>
      ) : (
        <>
          <div className="fixed top-0 left-1/4 w-[600px] h-[600px] bg-blue-400/[0.07] rounded-full blur-[130px] pointer-events-none z-0" />
          <div className="fixed top-1/3 right-1/4 w-[500px] h-[500px] bg-sky-400/[0.04] rounded-full blur-[120px] pointer-events-none z-0" />
        </>
      )}
      {/* Dot grid */}
      <div className={`fixed inset-0 pointer-events-none z-0 opacity-[0.018] ${isDark ? "bg-[radial-gradient(#60a5fa_1px,transparent_1px)]" : "bg-[radial-gradient(#1d4ed8_1.5px,transparent_1.5px)]"} [background-size:24px_24px]`} />
      ...
      <div className="relative z-10 flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top bar */}
        <header className={`sticky top-0 z-20 flex items-center gap-3 px-4 lg:px-6 py-3 border-b ${isDark ? "medical-surface-overlay border-b backdrop-blur-md" : "bg-white/80 border-slate-200 backdrop-blur-md"}`}>
          ...
          <div className="flex-1 min-w-0 flex items-center gap-3">
            <div className="min-w-0">
              ...
              <AdminFoundationLabel
                isDark={isDark}
                variant="header"
                className="hidden sm:block truncate min-w-0 mt-1"
              />
            </div>
          </div>
          ...
          <div className={`hidden sm:flex items-center gap-1.5 text-xs ${textMuted}`}>
            <div className={`w-1.5 h-1.5 rounded-full ${isRefreshing ? "bg-amber-400 animate-pulse" : "bg-sky-400"}`} />
          ...
          {/* Avatar */}
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold bg-gradient-to-tr from-blue-500 to-sky-400 text-slate-950 shrink-0 shadow-sm shadow-blue-500/20`}>
